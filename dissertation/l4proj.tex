...
Commonly used Secret Key Algorithms include Data Encryption Standard (DES), and Advanced Encryption Standard (AES) \citep{kessler_overview_2016}. 
...
RSA is a public key cryptographic algorithm invented by Ronald Rivest, Adi Shamir, and Leonard Adleman in 1978, and it depends on the hardness of factorization.
...
They have two important characteristics: knowing the hash, it must be difficult to find an input to the hash function so that it matches the hash;
it must also be difficult to find to input that generate the same hash \citep{savage_cse_2019}. 
...
There are several versions of this hash function (MD2, MD4, MD5) and vulnerabilities were found the MD5 algorithm such that collisions can v \citep{kessler_overview_2016}.
